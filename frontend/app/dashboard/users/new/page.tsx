...
export default function NewUserForm() {
  const [viewOption, setViewOption] = useState("show");
  ...
  const userTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { id } = e.target; // Get the id of the radio button
    setFormData((prev) => ({
      ...prev,
      isEmployee: id === "employee",
      isLocalAdmin: id === "dealerAdmin",
      isGlobalAdmin: id === "globalAdmin",
    }));
    switch (id) {
      case "employee":
        setViewOption("show");
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow mt-6">
      <h2 className="text-xl font-bold mb-4">New User Entry</h2>
      <form
        ...
        <div className="md:col-span-1 text-right">
          ...
          <label className="flex items-center space-x-2">
            <input
              ...
              radioGroup="userType"
              name="userType"
              checked={formData.isLocalAdmin}
              onChange={userTypeChange}
              className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
            />
            <span className="text-sm text-gray-700">Dealer Administrator</span>
          </label>
